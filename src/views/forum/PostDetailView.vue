<template>
    <div class="post-detail-app" ref="postDetailApp">
        <div class="inner">
            <div class="content">
                <div class="title"><span>创作</span>
                    <h1>{{ postDetail.postTitle }}</h1>
                </div>
                <div class="info">
                    <div>
                        <span class="type">帖子来自：</span>
                        <span class="type" v-if="postDetail.user?.userAccount == '17883351224'"
                            style="color:skyblue;">官方</span>
                        <span class="type" v-if="!postDetail.user?.userAccount == '17883351224'">{{
                                postDetail.user?.userName
                        }}</span>
                    </div>
                    <div title="浏览量">
                        <svg t="1666774022212" class="icon" viewBox="0 0 1024 1024" version="1.1"
                            ...
                            <path
                                ...
                                fill="#dbdbdb" p-id="2773"></path>
                        </svg>
                        <span>{{ postDetail.viewsTotal }}</span>
                    </div>
                    <div title="评论数">
                        <svg t="1666774103797" class="icon" viewBox="0 0 1071 1024" version="1.1"
                            xmlns="http://www.w3.org/2000/svg" p-id="3970" width="32" height="32">
                            <path
                                ...
                                p-id="3971" fill="#dbdbdb"></path>
                        </svg>
                        <span>{{ postDetail.commentTotal }}</span>
                    </div>
                    <div title="点赞量">
                        <svg t="1666774191659" class="icon" viewBox="0 0 1024 1024" version="1.1"
                            xmlns="http://www.w3.org/2000/svg" p-id="5172" width="32" height="32">
                            <path
                            ...
                            <path
                                ...
                                p-id="5176" fill="#dbdbdb"></path>
                        </svg>
                        <span>{{ postLikedInfo.total }}</span>
                    </div>
                </div>
                <div class="publish-time"><span>文章发表：{{ postDetail.publishTime }}</span></div>
                <div class="valueHtml" v-html="postDetail.postContent">

                </div>


                <ul class="toptic">
                    <li v-for="(item, index) in JSON.parse(postDetail.postTopics || '[]')" :key="index">{{ item }}</li>
                </ul>
            </div>
            <div class="comment" id="comment">
                <MyComment :commentInfo="{ postId: $route.query.postId, parentCommentId: -1, commentId: -1 }">
                </MyComment>
            </div>
            <div class="replay">
                <CommentDisplay :postId="$route.query.postId"></CommentDisplay>
            </div>
        </div>
        <div class="right">
            <div class="auth">
                <div class="avatar">
                    <img v-lazy='postDetail.user.userAvatar' alt="avatar" v-if="postDetail.user">
                </div>
                <div class="text">
                    <div v-if="postDetail.user">
                        <span class="nick-name">{{ postDetail.user.userName }}</span>
                        <span class="level">Lv.{{ postDetail.user.userLevel }}</span>
                    </div>
            ...
            <div class="code">
                <img src="https://bbs.mihoyo.com/_nuxt/img/qrcode_ys.c265aec.png" alt="二维码">
                <div>
                    <p>扫码下载米游社App</p>
                    <p>了解提瓦特最新资讯</p>
                </div>
            ...
            <div class="hot">
                ...
                <ul class="hot-list">
                    <li v-for="item in postList" :key="item.postId" @click="getPostDetail(item.postId)">
                        <div class="cover">
        ...
        <div class="left">
            ...
            <div title="收藏数" @click="collectUpdate">
                <svg t="1666777928817" class="icon" viewBox="0 0 1024 1024" version="1.1"
                    xmlns="http://www.w3.org/2000/svg" p-id="8296" width="32" height="32">
                    <path
                        ...
                        p-id="8297" fill="#cdcdcd"></path>
                </svg>
                <span>{{ collectInformation.totalCount }}</span>
            </div>
            <div class="liked" @click="updateLiked" title="点赞数">
                <svg t="1666777626658" class="icon" viewBox="0 0 1024 1024" version="1.1"
                    xmlns="http://www.w3.org/2000/svg" p-id="5532" width="32" height="32">
                    <path
...
<script setup>
import MyComment from '@/components/forum/MyComment.vue';
...
const postDetail = computed(() => _store.state.post.postDetail)
const postList = computed(() => _store.state.post.postList.slice(0, 6))

const likedInfo = {
    userAccount: "",
    type: 'post',
    id: _route.query.postId
}
const collect = {
    userAccount: null,
    type: 'post',
...
const getPostInformation = () => {
    ...
    _store.dispatch('post/getPostDetailByPostId', _route.query.postId)
    _store.dispatch('post/getPostLikedInfo', likedInfo)
    _store.dispatch('post/getCollectInfo', collect)
...
const postLikedInfo = computed(() => _store.state.post.postLikedInfo)
const postDetailApp = ref(null);

const updateLiked = () => {
    _store.dispatch('post/updateLikedInfo', likedInfo)
}
watch(() => postLikedInfo.value.isLiked, (newValue) => {
    if (newValue) {
        Array.from(postDetailApp.value.querySelectorAll('.left .liked svg path')).forEach((item) => {
...
onMounted(() => {
    if (postLikedInfo.value.isLiked) {
        Array.from(postDetailApp.value.querySelectorAll('.left .liked svg path')).forEach((item) => {
            item.classList.add('active')
        })
    }
...
const collectInformation = computed(() => _store.state.post.collectInformation)
const collectColor = computed(() => collectInformation.value.isCollect === false ? '#cdcdcd' : 'blue')

const collectUpdate = () => {
    _store.dispatch('post/updateCollectInfo', collect)
}
